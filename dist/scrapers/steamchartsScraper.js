...
exports.scrapeSteamchartsData = scrapeSteamchartsData;
 ...
 */
async function scrapeSteamchartsData() {
    let browser;
    try {
        ...
        console.log("[SteamchartsScraper] Waiting for '#top-games' element and its rows...");
        ...
        for (let i = 0; i < rowHandles.length; i++) {
            ...
            const initialRowData = await page.evaluate((row) => {
                ...
                const gameName = columns[1].querySelector("a")?.textContent?.trim() || "N/A";
                ...
                const hoursPlayed30Days = Array(30)
                    .fill(null)
                    .map(() => ({ Date: "", Hours: 0 }));
                return {
                    Rank: rank,
            ...
            if (initialRowData) {
                gamesData.push(initialRowData); // Cast to GameData
            }
            else {
                console.warn(`[SteamchartsScraper] Skipping row ${i + 1} due to insufficient columns.`);
            }
        }
        console.log(`[SteamchartsScraper] Initial data extracted for ${gamesData.length} games.`);
        ...
        if (!firstGameChartColumn) {
            console.error("[SteamchartsScraper] Could not find the chart column for the first game. Cannot extract 30-day data.");
        ...
        console.log(`[SteamchartsScraper] Found ${dayBarHandles.length} daily bars in the first game's chart.`);
        if (dayBarHandles.length === 0) {
            console.warn("[SteamchartsScraper] No daily bars found in the first game's chart. 30-day data will not be available.");
            return gamesData;
        }
        // Get the handle for the element that displays the date on hover
        const chartHeadElement = await page.$("#topgames-chart-head");
        if (!chartHeadElement) {
            console.error("[SteamchartsScraper] Could not find '#topgames-chart-head' element. Dates for 30-day data might be missing.");
        }
        // Loop through each day bar (up to 30 days)
        for (let dayIndex = 0; dayIndex < Math.min(dayBarHandles.length, 30); dayIndex++) {
            const dayBarHandle = dayBarHandles[dayIndex];
            console.log(`[SteamchartsScraper] Hovering over day ${dayIndex + 1} bar...`);
            ...
            let dateForDay = "";
            if (chartHeadElement) {
                dateForDay = await page.evaluate((el) => el.textContent?.trim() || "", chartHeadElement);
            }
            // Now, for this specific day, extract the 'Hours Played' value for ALL games
            for (let gameIdx = 0; gameIdx < gamesData.length; gameIdx++) {
                const hoursPlayedColumnHandle = await rowHandles[gameIdx].$("td.num.period-col.player-hours");
                if (hoursPlayedColumnHandle) {
                    const hoursText = await page.evaluate((el) => el.textContent?.trim() || "0", hoursPlayedColumnHandle);
                    ...
                    console.warn(`[SteamchartsScraper] Hours Played column not found for game ${gamesData[gameIdx].GameName} on day ${dayIndex + 1}.`);
        ...
        console.log("[SteamchartsScraper] Finished collecting 30-day hours data for all games.");
        return gamesData;
    }
    catch (error) {
        console.error(`[SteamchartsScraper] Error during scraping: ${error.message}`);
